...
| `python tools/train_overnight.py --walk --minutes 60` | Self-teaching trainer. **`--walk` (recommended): the bot WALKS between camera sweeps** (edge-safe; never breaks/places, world untouched) so it collects each block from many positions/distances/angles — position diversity is what fixes per-block confusion (e.g. oak_log) that a stationary spin can't. Without `--walk` it's camera-only (lighting diversity from day/weather). Retrains the CNN continuously; checkpoints metrics + a heartbeat (`data/metrics/overnight_status.json`); pauses on focus loss, resumes automatically; panic-stop Ctrl+Shift+X/End/Pause. Pair with `tools/eval_recognizer.py --log` + `tools/model_snapshot.py` to track/rollback the benchmark. |
