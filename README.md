...
| `python tools/test_treechop_live.py --logs 3` | **Autonomous tree-chopper**: scans → finds an oak log → walks to it (sprint; jumps/pillars out if stuck; mines through walls) → chops the trunk with the axe → collects → explores for the next. Mines logs only. Skills in `agents/skills.py` + `agents/treechop.py`; action map in `docs/bot_actions.md`. Panic: Ctrl+Shift+X. |
