...
def main() -> int:
    ...
    # Wrap everything in try/finally so a crash in the middle still
    # tears the mouse worker down. Otherwise the daemon thread keeps
    # spinning until Python exits, and any non-zero residual velocity
    # in flight can still produce a visible cursor shift after the
    # script prints its final line.
    try:
        # Read pose BEFORE
        frame = cap.get_frame()
        info0 = f3.read(frame)
        ...
        if y0 is not None and y2 is not None and p0 is not None and p2 is not None:
            # Reverse motion. mouse_per_degree ~6.5, so:
            dyaw = y0 - y2
